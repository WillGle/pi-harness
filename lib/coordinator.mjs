...
function taskPrompt(task) {
  ...
  if (task.owner === "worker") {
    return [
      ...
      "Implement the scoped change, run the verification command, and create exactly one atomic commit.",
      "The commit body must contain `Scope: ...` and `Reason: ...`.",
...
export async function executeCoordinateTask(pi, input, options = {}) {
  ...
  try {
    const spawnData = await requestRpc(pi, "subagents:rpc:spawn", {
      ...
      options: {
        description: `${task.owner}: ${truncateText(task.scope, 80).text}`,
        cwd: repo,
