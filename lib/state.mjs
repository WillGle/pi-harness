...
export function controlStateSummary({ goal, plan, decisions = [], changedFiles = [], gates = [], blocker = undefined }) {
  return { mission: goal ? { objective: goal.objective, status: goal.status } : undefined, plan: plan?.plan ?? [], decisions, changedFiles, gates, blocker, format: "agent-english-v1", updatedAt: new Date().toISOString() };
}

// Keep the old export for consumers during migration; it no longer compresses control state.
export const cavemanSummary = controlStateSummary;
