...
def call_claude(prompt: str) -> str:
    """Send a compression prompt to an LLM.

    Backend resolution, most to least specific:
      1. ``CAVEMAN_LLM_CMD`` — any command that reads the prompt on stdin and
         prints the completion on stdout (e.g. ``codex exec -``,
         ``gemini -p -``, ``ollama run llama3.2``). Lets non-Claude
         coordinators use this skill without the Anthropic stack.
      2. Anthropic SDK when ``ANTHROPIC_API_KEY`` is set.
      3. The ``claude --print`` CLI (handles desktop auth).
    ...
    """
    llm_cmd = os.environ.get("CAVEMAN_LLM_CMD")
    if llm_cmd:
        import shlex
        ...
        try:
            ...
            raise RuntimeError(f"CAVEMAN_LLM_CMD not found on PATH: {llm_cmd}")
        except subprocess.CalledProcessError as e:
            raise RuntimeError(f"CAVEMAN_LLM_CMD failed:\n{e.stderr}")
