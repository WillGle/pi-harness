...
- **6 Curated Skills**: Checksum-pinned skills (`architecture-diagram`, `project-scouting`, `caveman`, `ponytail`, `pi-coordinator`, `skill-hub`) verified against the private `skills-central` checkout at commit `8cc8358`. `architecture-diagram` is a distilled derivative; its upstream source hash and every packaged resource are pinned.
