...
- **7 Curated Skills**: Checksum-pinned skills (`ask-user`, `architecture-diagram`, `project-scouting`, `caveman`, `ponytail`, `pi-coordinator`, `skill-hub`) verified against the private `skills-central` checkout at commit `8cc8358`. `architecture-diagram` pins its upstream source hash and packaged resources; `ask-user` clarifies ambiguous user intent in or outside `/plan`.
