...
function blocked(order, reason, evidenceRefs = [], verifier = "semantic") {
  return { version: 1, task_id: order.task_id, verifier, status: "blocked", criteria: semanticCriteria(order).map((criterion) => ({ criterion, status: "not_checked", finding: reason, evidence_refs: [] })), summary: reason, evidence_refs: evidenceRefs };
}
...
export async function reviewTask(order, result, cwd, run, verifier = "semantic") {
  ...
  catch { return blocked(order, "The selected Evidence is missing, invalid, or too large.", [], verifier); }
  let event;
  try { event = await run(formatVerificationOrder(packet)); }
  catch { return blocked(order, verifier === "security" ? "The configured security-reviewer model is not available through the current Pi/provider configuration or the security-reviewer did not return a result." : "The semantic Verifier did not return a result.", packet.evidence.map((item) => item.reference), verifier); }
  const raw = truncateText(event?.result ?? "", MAX_REVIEW_BYTES);
  let reviewRef;
  try { reviewRef = storeEvidence({ cwd, taskId: order.task_id, kind: verifier === "security" ? "security_review" : "semantic_review", content: raw.text, truncated: raw.truncated }).reference; }
  catch { return blocked(order, "The Evidence Store could not persist the semantic Verifier result.", [], verifier); }
