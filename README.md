...
- **9 Curated Skills**: Checksum-pinned skills (`ask-user`, `requirement-check`, `architecture-diagram`, `drawio-modeling`, `project-scouting`, `caveman`, `ponytail`, `pi-coordinator`, `skill-hub`). Unmodified upstream-derived skills are verified against `skills-central` commit `8cc8358`; `architecture-diagram` pins its upstream source hash plus all packaged resources. Harness-adapted `project-scouting`, `pi-coordinator`, `caveman` and `ponytail` are local derivatives with packaged checksums, not upstream snapshots. Locally authored skills pin all packaged resources: `drawio-modeling` handles ERD, functional decomposition and UML views separately from product architecture; `ask-user` clarifies ambiguous needs/goals in or outside `/plan` without granting permissions or requiring a special UI tool; `requirement-check` checks the basis of consequential requirements before implementation.
