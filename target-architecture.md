...
**Current flow:** Main Pi calls `pi_harness_coordinate(owner, scope, verification, permission, model?)`. Harness validates the three allowed owners, builds a prompt, requests a background child through the pi-subagents RPC event bus, waits for its terminal event and returns JSON. Scout/research have read-only tools and no worktree; their `verification` is prompt text, not an executed gate. Worker uses package-managed worktree and commit; Harness runs the shell gate in that worktree before cleanup, checks commit count/message and returns a branch for separate integration. Group-scoped abort signals link active tasks to `/goal cancel`; there is no persisted task graph or independent verifier stage. Worker `success` combines terminal status, gate and commit policy, not mission acceptance. Child result text (up to 8 KB), gate output (8 KB) and diff (20 KB) enter the parent tool result; read-only child text enters up to 8 KB. Compaction saves goal/plan and supplied decision/file/gate summaries, not persistent project intelligence. `/learn` is user-written reference memory, not a scout cache.

**Minimum contract:** Main Pi is commander (L0 objective, constraints, decisions, DoD, status). Coordinator is an L1 reasoning responsibility of main Pi for now (operation plan, dependencies, concise results, retry/escalation). Optional heads may propose orders, never own spawn or cancellation. An L2 TaskOrder needs an objective and bounded scope, relevant context/constraints, role profile and permission, acceptance criteria, deliverables, verification, reporting limit; IDs/dependencies when coordinating several tasks. The current API accepts only the subset above, so the rest is doctrine, not an enforced schema. Scout, research, implementation, testing and review are role/capability profiles of an ExecutionUnit, not new process primitives; only scout/research/worker profiles exist today. No recursive fanout. Harness owns routing/policy, gate/result normalization, task status and cancellation linkage; pi-subagents owns child lifecycle, concurrency and worktree isolation. Pi owns model/provider/session/compaction. The L3 ExecutionUnit performs disposable work. Verifier independently checks acceptance with deterministic gates first, review agent only when needed. Persistent project intelligence, if built, should be replaceable Harness-owned state outside the repository; task-specific reconnaissance stays ephemeral.

**Promotion:** execution record (private evidence/artifacts) to TaskResult (ID, status, summary, changed paths, verification, evidence/artifact references and surprises) to L1 operational summary to L0 mission status. A package child terminating is `execution_complete`, not `verified`: accept only after criteria are checked; otherwise retry, block or fail. No raw logs/diffs/transcripts promoted by default. Preserve worker isolation, one package-managed commit, explicit user-approved integration and central cancellation. Skills describe this rule but do not yet enforce result filtering or task acceptance.

**Implementation delta (not yet implemented):**

| Component | Current | Next behavior / reason | Compatibility risk |
|---|---|---|---|
| `extensions/pi-harness.ts`, `lib/coordinator.mjs` | Flat owner/scope/verification/permission; direct spawn | Add backward-compatible TaskOrder ID, criteria, reporting policy and role-to-existing-profile mapping; validate permission/capability at dispatch. Explicit orders avoid prompt-only contracts. | Existing callers and tests depend on flat schema; retain it during migration. |
| `lib/coordinator.mjs`, `lib/worker-gate.mjs` | Inline bounded raw output/diff, combined `success` | Keep evidence in bounded Harness-owned private storage; return concise TaskResult plus references, distinct execution and verification statuses. Parent must not ingest L3 by default. | Callers use `result`, `diff`, `gateEvidence`, `success`; version/migrate response. Failed tasks must retain accessible evidence. |
