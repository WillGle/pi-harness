...
// Raw child output stays in the execution record. Never infer acceptance from
// a terminal event or from a single gate. No evidence reference is invented.
export function taskResult(order, record) {
  ...
  const verification_status = record.verificationRan && record.gatePassed === false && order.role === "worker" && execution_status === "execution_complete"
    ? "failed" : "not_verified";
  return {
