    ...
    for (const profile of ["default", "security"]) {
      ...
      reviews.push(await reviewTask(selectedOrder, preliminary, repo, async (prompt) => {
        ...
        try {
          ...
          if (profile === "security" && !options.modelRegistry?.getAvailable?.().some((item) => `${item.provider}/${item.id}` === model)) throw new Error("The configured security-reviewer model is not available through the current Pi/provider configuration.");
          const spawnData = await requestRpc(pi, "subagents:rpc:spawn", {
          ...
          const event = await waiter.promise;
          if (controller.signal.aborted) throw new Error("The semantic Verifier was cancelled.");
          if (profile === "security" && managerRecord(spawnData.id)?.invocation?.modelId !== model) throw new Error("The configured security-reviewer model is not available through the current Pi/provider configuration.");
          return event;
        } catch (error) { waiter.cancel(error); throw error; }
      }, verifier));
    }
