...
export function runWorkerVerification(worktreePath, task, options = {}) {
  ...
  // Keep captured stdout and stderr content unchanged before the bounded
  // legacy capture. Their stream interleaving is not available from spawnSync.
  const evidence = truncateText(`${gateRun.stdout ?? ""}${gateRun.stderr ?? ""}`, MAX_GATE_EVIDENCE_BYTES);
  return {
    worktreePath,
...
function changedPaths(cwd, baseSha, branch) {
  ...
  const result = spawnSync("git", ["-C", cwd, "diff", "--name-only", "-z", `${baseSha}...${branch}`], { encoding: "utf8" });
