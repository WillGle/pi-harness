...
export function operationBrief(operation, state, mission = undefined, graph = migrateTaskGraph(operation), registry = undefined, report = undefined) {
...
export function coordinatorPrompt(packet) {
  return ["The Coordinator must return one JSON CoordinatorDecision. Version: 1.",
    ...
    "Actions: dispatch, accept_task, reject_task, accept_criterion, consult_head, block, report. A HeadReport is advice. The Coordinator must issue a later decision to dispatch or accept.",
    "For dispatch return task {task_id, owner, scope, permission, verification, constraints?, acceptance_criteria?, review_evidence?, review_profile?}.",
...
export function parseCoordinatorDecision(raw, operation) {
  ...
  const fields = ["version", "operation_id", "action", "reason", "task", "task_id", "criterion", "evidence_refs", "blocked_action", "required_condition", "question", "head_id"];
  if (!shape(decision, fields) || decision.version !== 1 || decision.operation_id !== operation.operation_id || !["dispatch", "accept_task", "reject_task", "accept_criterion", "block", "report", "consult_head"].includes(decision.action) || typeof decision.reason !== "string" || !decision.reason.trim() || decision.reason.length > 300) throw new Error("CoordinatorDecision is invalid or belongs to a foreign Operation");
  const actionFields = { dispatch: ["task"], accept_task: ["task_id"], reject_task: ["task_id"], accept_criterion: ["criterion", "evidence_refs"], block: ["task_id", "blocked_action", "required_condition", "question"], report: [], consult_head: ["head_id"] };
  ...
  if (decision.action === "dispatch") {
    if (!shape(decision.task, ["task_id", "owner", "scope", "permission", "verification", "constraints", "acceptance_criteria", "review_evidence", "review_profile"]) || !operation.required_task_ids.includes(decision.task.task_id) || decision.task.scope?.length > 1000 || decision.task.verification?.length > 500) throw new Error("Coordinator proposed an unknown or oversized TaskOrder");
...
export async function runOperation(initial, callbacks, { mission, state = coordinatorState(initial), graph = migrateTaskGraph(initial), cwd = process.cwd(), signal, registry, headStates = {} } = {}) {
  ...
  for (; state.turns < MAX_COORDINATOR_TURNS && operation.status !== "complete";) {
    ...
    let raw;
    try { raw = await callbacks.turn(coordinatorPrompt(operationBrief(operation, state, mission, graph, registry, latestReport))); }
    ...
    try {
      if (decision.action === "consult_head") {
        ...
        graph = claimTask(graph, operation, task.task_id);
        ...
        save(); // Persist running before spawn.
        let result;
