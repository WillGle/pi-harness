...
Promote **TaskResult**, not transcripts: task ID, separate Execution Status and Verification Status, concise conclusion, changed paths when known, and artifact references when available. Keep raw files, logs, commands, retries and diffs in disposable execution context; inspect them only to resolve an escalation. The tool response promotes only the versioned TaskResult. The internal legacy record still includes bounded raw worker diff/gate output and read-only prose for direct callers. It has no durable Evidence references. Legacy `success` is a combined lifecycle/gate/commit check, not acceptance. Do not interpret it as Definition of Done. Summarize before further promotion; do not copy raw payloads into plans, memory, or compaction state.
