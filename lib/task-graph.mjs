...
export function runningTaskIds(graph, operation) {
  validateTaskGraph(graph, operation);
  return operation.required_task_ids.filter((id) => graph.nodes[id].scheduler_status === "running");
}
export function availableSlots(graph, operation, limit) {
  if (!Number.isInteger(limit) || limit < 1 || limit > 4) fail("invalid parallel limit");
  return operation.status === "open" ? Math.max(0, limit - runningTaskIds(graph, operation).length) : 0;
}
export function dispatchableTaskIds(graph, operation, limit) {
  return readyTaskIds(graph, operation).slice(0, availableSlots(graph, operation, limit));
}
export function schedulerSummary(graph, operation, limit = 2) {
  validateTaskGraph(graph, operation);
  ...
  return { ...result, parallel_limit: limit, available_slots: availableSlots(graph, operation, limit) };
}
export function claimTasks(graph, operation, ids, limit) {
  const ready = new Set(readyTaskIds(graph, operation));
  if (!Array.isArray(ids) || !ids.length || new Set(ids).size !== ids.length || ids.length > availableSlots(graph, operation, limit) || ids.some((id) => !ready.has(id))) fail("TaskOrder batch exceeds capacity or contains a TaskOrder outside the Scheduler ready set");
  let next = graph;
  for (const id of ids) next = replaceNode(next, id, { scheduler_status: "running", attempts: next.nodes[id].attempts + 1 });
  return validateTaskGraph(next, operation);
}
export function claimTask(graph, operation, id) {
  return claimTasks(graph, operation, [id], 1);
}
