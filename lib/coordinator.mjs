...
export function modelDispatch(task, env = process.env) {
  validateTask(task);
  const role = task.owner.toUpperCase();
  const model = env[`PI_HARNESS_${role}_MODEL`]?.trim() || undefined;
  const piProvider = env[`PI_HARNESS_${role}_PROVIDER`]?.trim() || undefined;
  return { model, piProvider, provider: piProvider ?? model?.split("/", 1)[0] ?? "default" };
}
...
export function startChild(task, options = {}) {
  ...
  if (options.piProvider) args.push("--provider", options.piProvider);
